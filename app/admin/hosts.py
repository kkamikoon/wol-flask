...
def host_add():
    ...
    # broadcast   = request.form.get("broadcast", type=int)
    ...
    flash(message="[Example] Success Alert.", category="error")
...
def host_up(host_idx):
    ...
    host = Host.query.filter_by(idx=host_idx).one_or_none()
    
    if host == None:
        return f"error";
    return f"Send magic packet to `{host.name}`."